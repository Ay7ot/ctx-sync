...
### Fixed
- Interactive command approval prompt was never shown — `docker start` and `restore` silently skipped all commands even without `--no-interactive`, because no default prompt function was wired up
- Added default enquirer-based prompt for command approval (select all / select individually / skip all) and per-command confirmation

## [1.3.2] — 2026-02-15

### Fixed
- `docker start` and `docker stop` now support `--path <dir>` for cross-machine path resolution — stored Docker Compose paths from another OS no longer cause failures
- `restore` Docker commands now use the resolved `localPath` instead of the stored compose directory when the stored path doesn't exist on the current machine
- Added `resolveDockerComposeDir()` helper with the same fallback chain as `resolveLocalPath()`: `--path` override → stored path → `cwd`

## [1.3.1] — 2026-02-15

### Fixed
- v1.3.0 npm publish shipped stale compiled output — this release includes the actual code changes
...
### Fixed
- Added README to npm package — previously the npmjs.com page showed no documentation
